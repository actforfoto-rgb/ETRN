...
proof={'branch':'dev/no-driver-lab-r1','trigger_commit':os.getenv('GITHUB_SHA'),'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'unit':unit,'android':device,'unit_passed':sum(x['passed'] for x in unit),'unit_total':len(unit),'android_passed':sum(x['passed'] for x in device),'android_total':len(device),'apk_sha256':hashlib.sha256(p.read_bytes()).hexdigest() if p.exists() else None,'evidence_kind':'SYNTHETIC_GATEWAY_AND_ANDROID_EMULATOR','live_saby_test':'NOT_RUN','real_goskey_signatures':'NOT_RUN','production_release':'BLOCKED','main_branch_and_drive_apk':'NOT_MODIFIED','limitations':['crypto Create/GetStatus full wire schema not confirmed','cross-document server capability not confirmed','signature verifier and durable Store are test doubles','BatchEngine finalization not yet wired to MainActivity or live Saby']}
